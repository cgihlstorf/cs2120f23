...
def a_set'' : Set Nat := { n: Nat | 0 < n ∧ n < 4}
...
--         cases tnb with
--         | inl l => nomatch l
--         | inr r => nomatch r --do you not have to break this down further recursively?
--         ⟩
...
example : 2 ∈ a_set \ b_set :=
  ...
  by
    intro h
    nomatch h --we can rely on Lean to infer this here
⟩
...
-- example : 3 ∉ a_set \ b_set := by
--   show ¬ (3 ∈ a_set ∧ 3 ∉ b_set)
--   exact  λ f : (3 ∈ a_set ∧ 3 ∉ b_set) => by
--          cases f with
--          | intro l => nomatch l.inr
--          | intro r => nomatch r.inr
