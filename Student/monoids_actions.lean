...
--tactics
--tacticals (tactis that use other tactics) (ex: repeat)
theorem rot_add_assoc':
  ∀ (a b c : Rotation), (a + b) + c = a + (b + c) :=
    by
      intros a b c --assumes you have values abc
      cases a
      repeat {
          cases b
          repeat {cases c
...
theorem zero_add_rot:
  ...
  --theorem: _____ :=
    --by
      --induction
      --<proof for zero> --this is the base case
      --intro n' --we have a proof for the existence of n', which is different from case analysis
      --intro h --inductive hypothesis
...
--proof:
  ...
  --proof of base case n = 0, use rfl
    -- ind Nat ∀ (P: Mat → Prop),
      --∀ n :Nat
      -- P 0 →
      --∀ n', Pn' → P(n' + 1)
